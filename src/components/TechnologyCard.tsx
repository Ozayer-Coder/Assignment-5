    ...
    <div className="flex flex-col p-5 border rounded-2xl border-[#F1F5F9]">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex justify-between items-center ">
          <div className="badge badge-ghost text-[#475569] text-[11px]">{technology.category}</div>
          <p className="text-[#64748B] font-medium text-[11px]">{technology.difficulty}</p>
          <p className="flex items-center gap-1">
            <FaStar className="text-[#FBBF24]" />
            {technology.rating}
          </p>
        </div>
        <button className="btn rounded-lg bg-[#0A0F1D] p-2.5 text-white font-medium text-[12px]">Add to Stack</button>
      </div>
    </div>
  );
}
